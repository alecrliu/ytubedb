...
def showChannels(page_num):
    ...
    channels_info = Channel.query.paginate(
        per_page=12, page=page_num, error_out=False)
    # Assuming a to_dict method on your model
    channels = [Channel_to_dict(channel) for channel in channels_info.items]
...
def showChannel(channelId):
    ...
    videos = [Video_to_dict(video) for video in Video.query.filter_by(
        channel_id=channelId).all()]
    playlists = [Playlist_to_dict(
        playlist) for playlist in Playlist.query.filter_by(channel_id=channelId).all()]
...
def oneVideo(videoId):
    ...
    playlists_dict = [Playlist_to_dict(
        playlist) for playlist in video.inPlaylist] if video.inPlaylist else []
...
def showPlaylist(page_num):
    ...
    playlists_info = Playlist.query.paginate(
        per_page=12, page=page_num, error_out=True)
    playlists_info = [Playlist_to_dict(playlists)
...
def playList(playlistId):
    ...
    channel_dict = Channel_to_dict(
        playlist_info.channels) if playlist_info.channels else None
    videos_dict = [Video_to_dict(
        video) for video in playlist_info.videos] if playlist_info.videos else []
